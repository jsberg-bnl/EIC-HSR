...
arc01y = line('y','yo12_cqs10','yo1_cqs10',db)
...
mat07y = line('y','yi7_int9_3','yi7_int7_2',db)
ins07y = line('y','yi7_int7_1','yi7_int4_1',db)
trp07y = line('y','yi7_trp3','yi7_trp1',db)
...
all_slots = line_info(rhic_slots) # Slots kept intact but used in isolation
all_lines = line_info(line_list) # RHIC lines
ps_lines = line_info(line_list+ir8_parts) # Things where I use the RHIC ps scheme
...
with open('rhic-ps.bmad',mode='w') as file_bmad, open('rhic-ps.madx',mode='w') as file_madx:
    ...
    comment_line("quadrupoles",file_bmad,file_madx)
    (psset,swns) = write_transfer('quadrupole',('b1_gradient','k1'),db,ps_lines.lnms,file_bmad,file_madx)
    write_ps_to_i(psset,swns,db,file_bmad,file_madx)
    comment_line("sextupoles",file_bmad,file_madx)
    (psset,swns) = write_transfer('sextupole',('b2_gradient','k2'),db,ps_lines.lnms,file_bmad,file_madx)
    write_ps_to_i(psset,swns,db,file_bmad,file_madx)
    comment_line("kickers",file_bmad,file_madx)
    (psset,swns) = write_transfer('hkicker',('bl_kick','kick'),db,ps_lines.lnms,file_bmad,file_madx)
    write_ps_to_i(psset,swns,db,file_bmad,file_madx)
    (psset,swns) = write_transfer('vkicker',('bl_kick','kick'),db,ps_lines.lnms,file_bmad,file_madx)
    write_ps_to_i(psset,swns,db,file_bmad,file_madx)
    comment_line("correctors",file_bmad,file_madx)
    (psset,swns) = write_transfer_cors(db,ps_lines.correctors,file_bmad,file_madx)
    write_ps_to_i(psset,swns,db,file_bmad,file_madx)
